

<div id="footer" class="content row" >
    <div id="instafeed"></div>

    <div id="list-destinations" class="footer-carousel col-xs-24">
            <!-- Slides -->
            <?php
            $term_args = array(
                'taxonomy' => array('category','themes','cities'),
                'hide_empty' => true,
                'fields' => 'all',
                'count' => true,
            );
            $term_query = new WP_Term_Query($term_args);
            foreach ( $term_query->terms as $term ) {
                $term_link = sprintf( 
                    '<div class="flickity--list-element"><a class="h3" href="%1$s" data-mouse="link-internal"><strong><span>#</span> %2$s</strong></a></div>',
                    esc_url( get_category_link($term->term_id) ),
                    esc_html( $term->name )
                );
                echo sprintf( esc_html__( '%s', 'textdomain' ), $term_link );
            }   
        ?>
    </div>    <div id="list-destinations-2" class="footer-carousel footer-carousel-2 col-xs-24">
            <!-- Slides -->
            <?php
            $term_args = array(
                'taxonomy' => array('category','themes','cities'),
                'hide_empty' => true,
                'fields' => 'all',
                'count' => true,
            );
            $term_query = new WP_Term_Query($term_args);
            foreach ( $term_query->terms as $term ) {
                $term_link = sprintf( 
                    '<div class="flickity--list-element"><a class="h3" href="%1$s" data-mouse="link-internal"><strong><span>#</span> %2$s</strong></a></div>',
                    esc_url( get_category_link($term->term_id) ),
                    esc_html( $term->name )
                );
                echo sprintf( esc_html__( '%s', 'textdomain' ), $term_link );
            }   
        ?>
    </div>    <div id="list-destinations-3" class="footer-carousel footer-carousel-3 col-xs-24">
            <!-- Slides -->
            <?php
            $term_args = array(
                'taxonomy' => array('category','themes','cities'),
                'hide_empty' => true,
                'fields' => 'all',
                'count' => true,
            );
            $term_query = new WP_Term_Query($term_args);
            foreach ( $term_query->terms as $term ) {
                $term_link = sprintf( 
                    '<div class="flickity--list-element"><a class="h3" href="%1$s" data-mouse="link-internal"><strong><span>#</span> %2$s</strong></a></div>',
                    esc_url( get_category_link($term->term_id) ),
                    esc_html( $term->name )
                );
                echo sprintf( esc_html__( '%s', 'textdomain' ), $term_link );
            }   
        ?>
    </div>

    <div class="col-xs-18 col-xs-offset-3" >
        <span id="footer-name" class="h1 scroll-reveal" data-lining>Quatre-vingt treize Degrés</span>
    </div>
    <div class="col-xs-11 col-xs-offset-1">
        <?php 
            $args = array(
            'depth'       => 0,
            'sort_column' => 'menu_order',
            'theme_location' => 'footer-menu',
            'menu_class'  => 'menu',
            'include'     => '',
            'exclude'     => '',
            'echo'        => true,
            'show_home'   => false,
            'link_before' => '',
            'link_after'  => ''
            );
            wp_nav_menu( $args ); ?>
    </div>
    <div id="copyright" class="col-xs-11">
        <p> ©93.Degrés – by Alexis & Agathe</p>
    </div>
</div>
</main>


